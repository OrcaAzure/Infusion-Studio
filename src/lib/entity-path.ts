...
export function ingredientPath(id: string) {
  if (isOfflineDemo()) return appPath(`/ingredients/item?id=${encodeURIComponent(id)}`);
  return appPath(`/ingredients/${id}`);
}

export function ingredientEditPath(id: string) {
  if (isOfflineDemo()) return appPath(`/ingredients/item/edit?id=${encodeURIComponent(id)}`);
  return appPath(`/ingredients/${id}/edit`);
}

export function blendPath(id: string) {
  if (isOfflineDemo()) return appPath(`/blends/item?id=${encodeURIComponent(id)}`);
  return appPath(`/blends/${id}`);
}

export function blendEditPath(id: string) {
  if (isOfflineDemo()) return appPath(`/blends/item/edit?id=${encodeURIComponent(id)}`);
