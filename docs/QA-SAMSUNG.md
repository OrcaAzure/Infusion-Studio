...
## For the developer — share with QA

### Option A: Public URL (best)

Deploy to Vercel (or similar) with `DATABASE_URL`, `AUTH_SECRET`, and `AUTH_URL` set. Send QA your URL and `/qa` for install help.

### Option B: Offline APK (no server)

```bash
npm run android:build:offline
```

APK output: `releases/InfusionStudio-Offline-Demo.apk` (gitignored — build locally).

### Option C: Remote-server APK

Point Capacitor at a live HTTPS URL, then:
